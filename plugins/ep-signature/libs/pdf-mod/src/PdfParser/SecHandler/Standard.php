...
class Standard extends SecHandler
{
     ...
     */
    public static function ensurePasswordEncoding(
        $revision,
        #[\SensitiveParameter]
        $password
    ) {
        if ($revision <= 4) {
            ...
            foreach ($chars as $char) {
                $newChar = $pdfDoc2Utf8[$char] ?? false;
                if ($newChar === false) {
    ...
    public function __construct(PdfParser $parser, PdfDictionary $encryptionDictionary)
    {
        ...
        $id = PdfDictionary::get($parser->getCrossReference()->getTrailer(), 'ID');
        if ($id instanceof PdfArray) {
            $this->fileIdentifier = self::getStringValue(PdfArray::ensure(
                $id,
                2
            )->value[0]);
        // there are still PDFs in the wild without a file-identifier
        } else {
            $this->fileIdentifier = '';
        }
     ...
     * @throws SecHandlerException
     */
    public function authByOwnerPassword(
        #[\SensitiveParameter]
        $password
    ) {
     ...
     * @throws SecHandlerException
     */
    public function authByUserPassword(
        #[\SensitiveParameter]
        $password
    ) {
        $password = self::ensurePasswordEncoding($this->getRevision(), $password);
     ...
     * @throws PdfTypeException
     */
    protected function createEncryptionKeyByUserPassword(
        #[\SensitiveParameter]
        $userPassword = ''
    ) {
     ...
     * @throws SecHandlerException
     */
    protected function createEncryptionKeyByOwnerPassword(
        #[\SensitiveParameter]
        $ownerPassword = ''
    ) {
        $revision = $this->getRevision();
    ...
    protected function computeUValue(
        #[\SensitiveParameter]
        $encryptionKey
    ) {
        $revision = $this->getRevision();
