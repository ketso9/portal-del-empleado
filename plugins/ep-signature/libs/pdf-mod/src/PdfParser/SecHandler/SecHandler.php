...
abstract class SecHandler
{
    ...
    /**
     * Flag that simulates OpenSSL 3 on PHP < 8.1 without legacy providers
     *
     * @var bool
     * @internal
     */
    public static $fakeInvalidOpenSslAndPhpVersion = false;

    /**
     * An array to remember checks for supported ciphers.
     *
     * @var array
     */
    public static $cipherChecks = [];
    ...
    public static function checkCipherSupport($cipher)
    {
        ...
        if (isset(self::$cipherChecks[$cipher])) {
            return self::$cipherChecks[$cipher];
        }
        ...
        $check = \in_array($cipher, \openssl_get_cipher_methods(), true);
        if (
            $cipher === 'rc4-40'
            && (
                ($check && \OPENSSL_VERSION_NUMBER >= 0x30000000 && \PHP_VERSION_ID < 80100)
                || self::$fakeInvalidOpenSslAndPhpVersion
            )
        ) {
            ...
            if (!$out || self::$fakeInvalidOpenSslAndPhpVersion) {
                $check = false;
            }
        }

        return self::$cipherChecks[$cipher] = $check;
    }
     ...
     * @throws SecHandlerException
     */
    public static function arcfour(
        #[\SensitiveParameter]
        $key,
        #[\SensitiveParameter]
        $data
     ...
     * @return string
     */
    protected function computeHashR6(
        #[\SensitiveParameter]
        $data,
        #[\SensitiveParameter]
        $inputPassword,
